...
3. Leave `RELAY_CLASSIFIER=local` and recreate the gateway with `docker compose up --build -d --wait` so environment changes take effect. Use the console or the same `/v1/chat/completions` request format as the mock setup. The public Vercel site serves documentation only and does not accept provider keys.
